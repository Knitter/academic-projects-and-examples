 ...
 */
package cusco.gui;
import cusco.AutenticationClient;
import cusco.Cusco;
 ...
 */
public class CuscoGUI extends JFrame implements GUIHandler {
	...
	// Classe com o código do cliente
	    ...
	    jbSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (jListServers.getSelectedIndex() == -1) {
					showErrorMessage("Pesquisar", "Tem de seleccionar pelo menos 1 servidor.");
					return;					
				}
				...
				Thread t = new Thread() {
					public void run() {
						doReset = true;
						tableModel.clear();
						client.search(login, passw, pattern, securitas, servers, false);
						jbSearch.setEnabled(false);
	...
	public void addServer() {
		...
		try {
			...
			if (serverNames.contains(servername)) {
				showErrorMessage("IP inválido", "Endereço IP duplicado.");
				return;			
			}
