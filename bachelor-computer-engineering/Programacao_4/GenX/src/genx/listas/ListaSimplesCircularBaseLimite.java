...
public class ListaSimplesCircularBaseLimite implements ListaSimplesADT, java.io.Serializable {
    ...
    public boolean reordenarElemento(Object elem) {
        NoListaSimplesCircularBaseLimite ant = noInicial;
        while (ant.seguinte != noInicial && elem != ant.seguinte.elemento)
            ant = ant.seguinte;

        if (elem != ant.seguinte.elemento)
            return false;
    ...
    public Iterador consultar(Object elem) {
        NoListaSimplesCircularBaseLimite ant = pesquisar(elem);
        ListaSimplesCircularBaseLimite lc = new ListaSimplesCircularBaseLimite(
            criterio);
        while (criterio.comparar(elem, ant.seguinte.elemento) == 0) {
    ...
    private NoListaSimplesCircularBaseLimite pesquisar(Object elem) {
        NoListaSimplesCircularBaseLimite ant = noInicial;
        while (criterio.comparar(elem, ant.seguinte.elemento) > 0)
            ant = ant.seguinte;
        return ant;
...
class NoListaSimplesCircularBaseLimite implements java.io.Serializable {
    ...
    NoListaSimplesCircularBaseLimite(Object elem,
                                     NoListaSimplesCircularBaseLimite seg) {
        elemento = elem;
        seguinte = seg;
    }
...
class IteradorListaSimplesCircularBaseLimite implements Iterador {
    ...
    private NoListaSimplesCircularBaseLimite noInicial, corrente;

    IteradorListaSimplesCircularBaseLimite(NoListaSimplesCircularBaseLimite
                                           noInicial) {
